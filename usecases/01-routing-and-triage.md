...
- [xinyao27/jevonian](https://github.com/xinyao27/jevonian) is a local proxy that answers OpenAI, Anthropic, and Responses shapes and routes the virtual model `jevonian/auto` with one Jev call that picks both the model and the thinking level. The part that matches this page's design notes: code filters candidates by wire protocol, context window, and an effort floor, and removes providers whose quota window is spent, **before** Jev is asked, and the confidence gate is on the route -- a pinned model or explicit `jevonian/<route>` skips Jev entirely, `routing.brainPicksEffort: false` drops the effort question, and a low-confidence verdict is marked (`x-jevonian-brain: jev-low-confidence`) rather than accepted silently. No measured routing result is published, so read it as an architecture example, not a benchmark; the ledger's costs are estimates from a price table. [self-reported by the author](https://github.com/xinyao27/jevonian/blob/main/docs/brain.md)
