...
| [kylemclaren/jevsearch](https://github.com/kylemclaren/jevsearch) | Site search as a shadcn/ui registry block (distinct from superagents-lab/jev-search). A local keyword pass answers on the first keystroke, then the top 20 hits go to Jev in one request: a Noul per candidate ("would the visitor be glad to land here?"), a Choice for the single best page, and a Noul for "does any page answer this at all?". Hits are re-ordered by a blend of relevance and best-answer share; if TypeSafe is slow or down, keyword order stands. No embeddings, no vector DB | Measured, self-reported by the author in [`bench/results.json`](https://github.com/kylemclaren/jevsearch/blob/1df37decb960b3c4826c71b41d3393c15a8f28d3/bench/results.json): Hit@1 83% vs 41% for its own keyword pass and 41% for Lunr, 41 labelled queries over the TypeSafe docs (109 pages); median 278 ms uncached; about $0.26 per 1,000 uncached searches. Launch-week artifact. Demo at [jevsearch.fly.dev](https://jevsearch.fly.dev) |
